...
export function ConsultationRoom({
  ...
  return (
    ...
    <div className="consultation-room-container">
      ...
      <DraggablePanel
        isOpen={showAnamnesePopup}
        onClose={() => { setShowAnamnesePopup(false); setSelectedAnamneseIndex(-1); }}
      ...
      >
        <div style={{ display: 'flex', height: '100%', margin: '-16px', overflow: 'hidden' }}>
          ...
          <div style={{ width: 160, borderRight: '1px solid #E2E8F0', overflowY: 'auto', background: '#F8FAFC', flexShrink: 0 }}>
            <div style={{ padding: '10px 12px', fontSize: 9, fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Selecione uma data</div>
            {allAnamneses.map((an: any, idx: number) => (
              ...
              >
                <span style={{ fontSize: 12, fontWeight: 700, color: selectedAnamneseIndex === idx ? '#1B4266' : '#0F172A' }}>
            ...
            {selectedAnamneseIndex < 0 || !allAnamneses[selectedAnamneseIndex] ? (
              ...
              const sections = [
                { title: 'Dados Pessoais', items: [
                  ...
                  { label: 'Data de Nascimento', value: a.data_nascimento }, { label: 'Sexo', value: a.genero }, { label: 'Profissao', value: a.profissao },
                  { label: 'CPF', value: a.cpf }, { label: 'Tipo Sanguineo', value: a.tipo_saguineo },
                ]},
                { title: 'Medidas', items: [
                ...
                { title: 'Sono, Agua e Jejum', items: [
                  ...
                  { label: 'Consumo de Agua', value: a.consumo_agua }, { label: 'Cor da Urina', value: a.cor_urina },
                  { label: 'Pratica Jejum', value: a.pratica_jejum }, { label: 'Duracao Jejum', value: a.duracao_jejum },
                ]},
                { title: 'Objetivo e Atividade Fisica', items: [
                  ...
                  { label: 'Nivel', value: a.nivel_atividade }, { label: 'Modalidades', value: Array.isArray(a.modalidades) ? a.modalidades.join(', ') : a.modalidades },
                  { label: 'Frequencia', value: a.frequencia_semanal }, { label: 'Periodo', value: a.periodo_treino },
                ]},
                ...
                { title: 'Saude e Medicamentos', items: [
                  { label: 'Toma Medicamentos', value: a.toma_medicamentos }, { label: 'Medicamentos', value: a.medicamentos_detalhes },
              ...
              return (<>
                {sections.map((section) => {
                  const validItems = section.items.filter((i: any) => i.value);
                  if (validItems.length === 0) return null;
                  return (
                    <div key={section.title} style={{ marginBottom: 16 }}>
                      <h3 style={{ fontSize: 12, fontWeight: 700, color: '#1B4266', marginBottom: 8, paddingBottom: 5, borderBottom: '1.5px solid #F1F5F9', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
                        {section.title}
                      </h3>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px 16px' }}>
                        {validItems.map((item: any) => (
                          <div key={item.label} style={{ padding: '4px 0' }}>
                            <div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 600, marginBottom: 1 }}>{item.label}</div>
                            <div style={{ fontSize: 12, color: '#0F172A', fontWeight: 500, lineHeight: 1.4 }}>{item.value}</div>
                          </div>
                        ))}
                      </div>
                    </div>
